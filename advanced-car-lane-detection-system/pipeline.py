...
def color_unwarped_lane(warped, img_size, left_fitx, right_fitx, ploty, matP_inv):
  ...
  # Draw the lane onto the warped blank image
  cv2.fillPoly(color_warp, np.int_([pts]), random_color())
...
def paste_curvature_and_offset(image, curverad, offset):
  ...
  image = cv2.putText(image, "lane curvature: " + str(curverad) + " meters", (400, 40), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
  image = cv2.line(image, (200, 70), (1020, 70), random_colors(), 2)
  image = cv2.putText(image, "car deviation: " + str(round(offset, 3)) + " meters", (400, 120), font, 1, (255, 255, 255), 2, cv2.LINE_AA)

  image = cvzone.overlayPNG(image, imgFront, [630, 580])
  image = cvzone.overlayPNG(image, imgFront2, [630, 540])
...
if __name__ == '__main__':
  clip1 = VideoFileClip("./miami1.mp4")
